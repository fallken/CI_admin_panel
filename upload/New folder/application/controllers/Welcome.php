...
class Welcome extends CI_Controller {
	...
	public function edit_post($post_id){
		...
		if (!isset($input)){
			$dada['post_info']=$this->main->edit_post_get_info($post_id);
			...
			if (unlink('./'.$path->img)){
				...
				$this->session->set_flashdata('update_failure','could not delete the image'.' '.base_url().'index.php/'.$path->img);
			}
			$image=$this->upload_image();
				$update=array(
...
//				if($time2-$this->seconds_to_time($item->time)<=7){
//					echo $time2-$this->seconds_to_time($item->time);
//					echo '<br>';
//
//				}}
			echo $time2-$this->seconds_to_time($item->time);
