...
//		}
	...
	public function post_rank_weekly(){
		$query = $this->db->query('SELECT id,time,views,title FROM posts ORDER BY time DESC,views DESC LIMIT 20');//this query will be used to create related json responses in the program
		return $query->result();
	}
    ...
    public function update($post_id,$update){
	    $this->db->where('id',$post_id);
	     if($this->db->update('posts',$update))return true;
	     else return false;
